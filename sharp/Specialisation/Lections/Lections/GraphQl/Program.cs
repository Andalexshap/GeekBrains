...
public record BookInput(string title, int authorId);
...
public class Mutation
{
    ...
    {
        var author = await repository.GetAuthor(input.authorId) ?? throw new Exception("Author not found");
...
public class Repository
{
    ...
    {
        new Book(0,"Книга 0", authors[0]),
        new Book(1,"Книга 1", authors[1]),
        new Book(2,"Книга 2", authors[2]),
        new Book(3,"Книга 3", authors[2])
    };
    ...
    {
        return Task.FromResult(books.Where(x => x.id == authorId).ToList());
    }
    public Task<Author?> GetAuthor(int authorId)
    {
        return Task.FromResult(authors.FirstOrDefault(x => x.id == authorId));
    }
